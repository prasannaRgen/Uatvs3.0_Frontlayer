var show = {
    ...
    showData: function (pageName) {
        ...
        //        break;
        //}
        var msg = ServiceLayer.GetData("GetDropdownDataTestManagement"+ "/"+ _spUserId + "/" + pageName,"","GroupPortfolio");
        if (msg != undefined && msg != null) {

            /**/
            var newData = msg;
            for (var i = 0; i < msg.length; i++) {
                msg[i].leadEmailId = (msg[i].leadEmailId!="") ? $($.parseXML(msg[i].leadEmailId)).find("User>userEmail").text() : "";
                msg[i].roleList = '<root>' + msg[i].roleList + '</root>';
                var roleLen = $($.parseXML(msg[i].roleList)).find('role>roleId').length;
                var rolearr = []
                for (var j = 0; j < roleLen; j++) {
                    rolearr.push({
                        "roleId": $($($.parseXML(msg[i].roleList)).find('role>roleId')[j]).text(),
                        "roleName": $($($.parseXML(msg[i].roleList)).find('role>roleName')[j]).text(),
                        "roleDetails": '',
                        "isTestersAssigned": (parseInt($($($.parseXML(msg[i].roleList)).find('role>testerAssignedCount')[j]).text())==0)?"0":"1"
                    });
                }
                msg[i].roleList = rolearr;
               // msg[i].testPassList.roleList = rolearr;
                
                var countTp = (msg[i].testPassList == null || msg[i].testPassList == undefined) ? 0 : msg[i].testPassList.length;
                for (var k = 0; k < countTp; k++) {
                    msg[i].testPassList[k].testCaseList = '<root>' + msg[i].testPassList[k].testCaseList + '</root>';
                    var tcArr = []
                    var tcLen = $($.parseXML(msg[i].testPassList[k].testCaseList)).find('testcase>testCaseId').length;
                    for (var j = 0; j < tcLen; j++) {
                        tcArr.push({
                            "testCaseId": $($($.parseXML(msg[i].testPassList[k].testCaseList)).find('testcase>testCaseId')[j]).text(),
                            "testCaseName": $($($.parseXML(msg[i].testPassList[k].testCaseList)).find('testcase>testCaseName')[j]).text()
                        });
                    }
                    msg[i].testPassList[k].testCaseList = tcArr;


                    msg[i].testPassList[k].listRoles = '<root>' + msg[i].testPassList[k].listRoles + '</root>'
                    var roleArr = []
                    var roleLen1 = $($.parseXML(msg[i].testPassList[k].listRoles)).find('role>roleId').length;
                    for (var j = 0; j < roleLen1; j++) {
                        roleArr.push({
                            "roleId": $($($.parseXML(msg[i].testPassList[k].listRoles)).find('role>roleId')[j]).text(),
                            "roleName": $($($.parseXML(msg[i].testPassList[k].listRoles)).find('role>roleName')[j]).text(),
                            "isTestersAssigned": (parseInt($($($.parseXML(msg[i].testPassList[k].listRoles)).find('role>testerAssignedCount')[j]).text()) == 0) ? "0" : "1"
                        });
                    }
                    msg[i].testPassList[k].listRoles = roleArr;
                }


                //*** for StakeholderList
                if (msg[i].hasOwnProperty("stakeholderList") && msg[i].stakeholderList != "") {
                    msg[i].stakeholderList = '<root>' + msg[i].stakeholderList + '</root>'
                    var arr1 = []
                    var roleLen1 = $($.parseXML(msg[i].stakeholderList)).find('user>spUserId').length;
                    for (var j = 0; j < roleLen1; j++) {
                        arr1.push({
                            "stakeHolderEmail": $($($.parseXML(msg[i].stakeholderList)).find('user>userEmail')[j]).text()
                                //$($.parseXML(msg[i].stakeholderList)).find('user>userEmail').text()
                        });
                    }
                    msg[i].stakeholderList = arr1;
                ...
                if (isPortfolioOn) {
                    if (projectName[i].groupName != null && projectName[i].groupName != undefined) {
                        if (show.projectId == projectName[i].projectId) {
                            ...
                            show.selectedProjectLeadEmailId = projectName[i].leadEmailId;//$($.parseXML(projectName[i].projectLead)).find("User>userEmail").text(); //projectName[i].leadEmailId;//Added By Mohini DT:30-07-2014
                            show.selectedProjectStakeholdersEmailId = show.JoinStakeholderArr(projectName[i].stakeholderList); //Added By Mohini DT:05-08-2014
    ...
    JoinStakeholderArr: function (lst) {
        var _joinedEmailIds = ""
        if (lst == null || lst.length == 0) {
            return _joinedEmailIds;
        }
        for (var i = 0; i < lst.length; i++) {
            _joinedEmailIds = lst[i].stakeHolderEmail+";"
        }
