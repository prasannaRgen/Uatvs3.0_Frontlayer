...
var attachment = {
        ...
        if (!filename) {
            ...
            if (checked < 1) {
                ...
                if (attachment.flagFound == "true") {
                    ...
                    //var queryVal = pid+";"+tpid+";"+tcid+";"+splitTSID+";"+0;
            ...
            if (attachment.result.length != 0) {
                for (var j = 0; j < attachment.result.length; j++) {
                    if (attachment.result[j]['testCaseId'] == $("#testCaseName").val() && (attachment.result[j]['listExpectedAttach'].length != 0)) {
                        if ($.inArray(attachment.result[j]['listExpectedAttach'][0]['filePath'], uniqueExpUrl) == -1) {
                            uniqueExpUrl.push(attachment.result[j]['listExpectedAttach'][0]['filePath']);//Unique URLs

                            //list of testStepNames 
                            testStepNamesByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']] = new Array();
                            testStepNamesByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']].push(attachment.result[j]['testStepName']);

                            //list of testStepIds 
                            testStepIdsByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']] = new Array();
                            testStepIdsByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']].push(attachment.result[j]['testStepId']);

                            attachment.tempResult.push(attachment.result[j]['testStepId']);

                        }
                        else {
                            if ($.inArray(attachment.result[j]['testStepName'], testStepNamesByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']]) == -1)
                                testStepNamesByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']].push(attachment.result[j]['testStepName']);

                            if ($.inArray(attachment.result[j]['testStepId'], testStepIdsByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']]) == -1)
                                testStepIdsByExpectedResArr[attachment.result[j]['listExpectedAttach'][0]['filePath']].push(attachment.result[j]['testStepId']);
                    ...
                    if (attachment.result[j].length != 0)//To restrict entry of Null value in the 'attachment.attachTestStep' global array 
                    {
                        ...
                        if (attachment.result[j][0]['testCaseId'] == $("#testCaseName").val()) {
                            ...
                            if (attachment.result[j][0]['listExpectedAttach'].length != 0) {
                                if ($.inArray(attachment.result[j][0]['listExpectedAttach'][0]['filePath'], temp) == -1) {
                                    temp.push(attachment.result[j][0]['listExpectedAttach'][0]['filePath']);
                                    attachment.attachTestStep.push({
                                        "roleId": 'N/A',
                                        "roleName": 'N/A',
                                        "testCaseId": attachment.result[j][0]['testCaseId'],
                                        "testerName": 'N/A',
                                        "testerSpUserId": 'N/A',
                                        "testStepId": testStepIdsByExpectedResArr[attachment.result[j][0]['listExpectedAttach'][0]['filePath']].join(),
                                        "testStepName": testStepNamesByExpectedResArr[attachment.result[j][0]['listExpectedAttach'][0]['filePath']].join(),
                                        "testStepPlanId": attachment.result[j][0]['testStepPlanId'],
    ...
    editAttachment: function (fileIndex, fileUrl, testStepIds, testStepPlanId, fileType, description, attachID, attName) {
        ...
        var Result = attachment.dmlOperation(query, 'Attachment');
        if (Result != undefined)
		    ...
		    buttons:
					{
					    "Ok": function () {
					        if (attachment.noprjFlag == 1)
					            ...
					            window.location.href = Main.getSiteUrl() + '/SitePages/TestStep_1.aspx' + '?pid=' + show.projectId + '&tpid=' + show.testPassId;
					        $(this).dialog("close");
					    },
					    "Cancel": function () {
					        $(this).dialog("close");
